...
from sample.ap.utils.chemical import *


import sample.ap.rf2aa.kinematics as kinematics
import sample.ap.rf2aa.util as rfaautil
...
def rigid_from_3_points(N, Ca, C, non_ideal=False, eps=1e-8):
    ...
    if non_ideal:
        ...
        cos2del = torch.clamp(cosref * costgt + torch.sqrt((1 - cosref * cosref) * (1 - costgt * costgt) + eps),
                              min=-1.0, max=1.0)
...
def get_tips(xyz, seq):
    ...
    xyz_tips = torch.gather(xyz, 2, tip_indices.to(xyz.device)[seq][:, :, None, None].expand(-1, -1, -1, 3)).reshape(B,
                                                                                                                     L,
                                                                                                                     3)
...
def get_mu_xt_x0(xt, px0, t, schedule, alphabar_schedule, eps=1e-6):
    ...
    b = ((torch.sqrt(1 - schedule[t_idx] + eps) * (1 - alphabar_schedule[t_idx - 1])) / (
                1 - alphabar_schedule[t_idx])) * xt_ca
    mu = a + b
...
def get_t2d(xyz_t, is_sm, atom_frames):
    ...
    '''
    L = xyz_t.shape[1]
    xyz_t_frames = rfaautil.xyz_t_to_frame_xyz_sm_mask(xyz_t[None], is_sm, atom_frames[None])
    mask_t_2d = torch.ones(1, L, L).bool().to(xyz_t_frames.device)
    t2d = kinematics.xyz_to_t2d(xyz_t_frames, mask_t_2d[None], params=rf2aa.kinematics.PARAMS | {'USE_CB': True})
